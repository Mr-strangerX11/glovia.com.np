...
export default function CreateFlashDealPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to create flash deal');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Create Flash Deal</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Add a new flash deal with multiple products</p>
        </div>
      </div>
