...
function NewProductContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isObjectId(formData.vendorId)) {
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  <option value="">{vendorsLoading ? 'Loading vendors…' : '— Optional —'}</option>
                  {Array.isArray(vendors) && vendors.map((vendor: any) => {
                    const vendorName = vendor.firstName || vendor.email;
                    const vendorId = vendor._id || vendor.id;
                    return (
                      <option key={vendorId} value={vendorId}>{vendorName}</option>
                    );
                  })}
                </select>
                {formData.vendorId && (
